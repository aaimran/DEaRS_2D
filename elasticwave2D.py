#!/usr/bin/env python3
"""
2D Elastic Wave Equation Earthquake Rupture Simulation
SBP-SAT finite difference method in velocity-stress form

Usage:
    python3 elasticwave2D.py input.in [--saveplots] [--savefields]
"""
...
def run_simulation(params):
    """
    Run the 2D elastic wave equation simulation.
    ...
    print("\n" + "="*60)
    print("STEP 3: INITIALIZING FIELDS AND GRIDS")
    print("="*60)
    ...
    # Time-stepping loop
    print("\n" + "="*60)
    print("STEP 7: TIME-STEPPING SIMULATION")
    print("="*60)
    print(f"  Running {nt} time steps (dt={dt:.6f} s)...")
    print(f"  Progress updates every {isnap} iterations\n")
    ...
    for it in range(nt):
        ...
        if it % isnap == 0:
            percent = 100 * it / nt
            print(f'  [{percent:5.1f}%] t = {t:.3f} s, iteration {it:4d}/{nt}')
    
    print(f'  [100.0%] t = {nt*dt:.3f} s, iteration {nt:4d}/{nt}')
    print("\n" + "="*60)
...
def save_plots(results, output_prefix='sim'):
    ...
    def update_fault(frame_it):
        ...
        ax3.set_title(f"Time: {frame_it*dt:.2f} s")
        return [line3, line4, line5]
    
    frames_fault = range(0, nt - 1, 10)
    ...
    print("\n[4/5] Creating slip rate space-time plot...")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='2D Elastic Wave Equation Earthquake Rupture Simulation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 elasticwave2D.py default.in
  python3 elasticwave2D.py default.in --saveplots
  python3 elasticwave2D.py default.in --saveplots --savefields
        """
    )
    ...
    print("\n" + "="*60)
    print("2D ELASTIC WAVE EQUATION SIMULATION")
    print("="*60)
