def elastic_rate2d(D_l, F_l, Mat_l, X_l, Y_l, t, nf, nx, ny, dx, dy, order, r_l, source_parameter, D_r, F_r, Mat_r, X_r, Y_r, r_r, friction_parameters, slip, psi, dslip, dpsi,fric_law, FaultOutput0, stage, Y0):
    ...
    for i in range(0, nx):
        for j in range(0, ny):
            ...
            x_l = X_l[i,j]
            y_l = Y_l[i,j]
            Ft_l = pointSource(dx, dy, i, j, x_l, y_l, t, source_parameter)
            ...
            x_r = X_r[i,j]
            y_r = Y_r[i,j]
            Ft_r = pointSource(dx, dy, i, j, x_r, y_r, t, source_parameter)
...
def interfacecondition2d(F_l, F_r, Fhat_l, Fhat_r, Mat_l, Mat_r, nx, ny, friction_parameters, slip, psi, dslip, dpsi, fric_law, FaultOutput0, Y, t, stage, Y0): 
    ...
    for j in range(0, ny):
        ...
        return_int = interfacedata.friction_law(vy_l, Ty_l, vy_r, Ty_r, slip[j, 0], psi[j, 0], rho_l, mu_l, rho_r, mu_r, friction_parameters_index, fric_law)
        vy_m = return_int['V_m']
        vy_p = return_int['V_p']
        Ty_m = return_int['T_m']
        Ty_p = return_int['T_p']
...
def pointSource(dx, dy, ix, jy, x, y, t, source_parameter):
